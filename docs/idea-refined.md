...
> This is the original, raw idea — preserved as the source of truth. The distilled,
> design-open version lives in [`vision.md`](vision.md) and the seam docs alongside it.
...
> Note (design-open restart): the stack above is the *original intent*. In this restart it
> is treated as a set of leading candidates with explicit constraints — see
> [`open-decisions.md`](open-decisions.md). Apps are built standalone first, then combined.
